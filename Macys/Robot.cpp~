...
//Resets gyro to zero when crab starts
	...
	if(Robot::oi->getDriverJoystick()->GetRawButton(1)) //Crab if trigger pulled
	{
		if(Robot::oi->getDriverJoystick()->GetMagnitude() < .1) //Implements directional deadband
		{
			if(Robot::oi->getDriverJoystick()->GetZ() < .1 && Robot::oi->getDriverJoystick()->GetZ() > -.1) //Implements twist deadband
			{
				Robot::driveTrain->Crab(0,0,0,true); //No twist or movement
			}
			else
			{
				Robot::driveTrain->Crab(Robot::oi->getDriverJoystick()->GetZ()/2,0,0,true); //Only twist, no movement
			}
		}
		else
		{
			Robot::driveTrain->Crab(Robot::oi->getDriverJoystick()->GetZ()/2,-Robot::oi->getDriverJoystick()->GetY(),Robot::oi->getDriverJoystick()->GetX(),true); //twist and movement
		}
	}
	else //Steering if trigger not pulled
	{
		if(Robot::oi->getDriverJoystick()->GetMagnitude()<.2) //Implements deadband
			Robot::driveTrain->Steer(pi/2,0,.5); // steers wheels straight with no speed if within deadband
		else
		{
			if(Robot::oi->getDriverJoystick()->GetY()<0) //steering, speed changes direction depending on Y value.
					Robot::driveTrain->Steer(Robot::oi->getScaledJoystickRadians(),-Robot::oi->getDriverJoystick()->GetMagnitude(),0.5);
				else
					Robot::driveTrain->Steer(Robot::oi->getScaledJoystickRadians(),Robot::oi->getDriverJoystick()->GetMagnitude(),0.5);
		}
	}
